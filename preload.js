 ...
 *
 * Rules enforced:
 *  - contextIsolation: true  → window.telvault is the ONLY bridge
 *  - sandbox: true           → no require() in renderer or preload
 *  - Only explicitly listed channels are forwarded (allowlist pattern)
 */
...
contextBridge.exposeInMainWorld('telvault', {
  ...
  onboarding: {
    hasCredentials:  ()        => invoke('onboarding:hasCredentials'),
    saveCredentials: (payload) => invoke('onboarding:saveCredentials', payload),
  },
